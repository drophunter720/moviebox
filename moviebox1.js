...
function gwOpenGatewayBtn(btn) {
    gwOpenGateway(btn.dataset.id, btn.dataset.type, btn.dataset.title, btn.dataset.season, btn.dataset.ep);
}
(() => {
    document.addEventListener("DOMContentLoaded", () => {
        function Y(P) {
            ...
            async function M() {
                try {
                    let [R, s] = await Promise.all([
                        fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${P}&language=en-US`).then((l) =>
                            l.json()
                        ),
                        fetch(`https://api.themoviedb.org/3/genre/tv/list?api_key=${P}&language=en-US`).then((l) =>
                            l.json()
                        ),
                    ]);
                    ...
                    v.forEach(async (E) => {
                        ...
                        <div class="search-info">
                            ...
                            L.addEventListener("click", () => {
                                window.location.href = `/p/info.html?id=${E.id}&type=${E.media_type}`;
                            }),
                            _.appendChild(L);
                    }),
            ...
            if (z) {
                let S = function () {
                        ...
                        (F.innerHTML += Object.entries(J)
                            .map(([k, c]) => `<option value="${k}">${c}</option>`)
                            .join("")),
                            [x, H, O, F, L, I].forEach((k) => {
                    ...
                    T = function (B = !1) {
                        ...
                        let k = x.value,
                            ...
                            i = p.length >= 2,
                            $ = i
                                ? `https://api.themoviedb.org/3/search/${k}?api_key=${P}&language=en-US&query=${encodeURIComponent(p)}&page=${R}`
                                : `https://api.themoviedb.org/3/discover/${k}?api_key=${P}&language=en-US&sort_by=popularity.desc&page=${R}`;
                        f && ($ += `&vote_average.gte=${f}`),
                            i ||
                                (c && ($ += `&primary_release_year=${c}`),
                                d && ($ += `&with_genres=${d}`),
                                m && ($ += `&with_origin_country=${m}`)),
                            fetch($)
                                .then((y) => y.json())
                                .then((y) => {
                        ...
                        <div class="movie-card" data-id="${h.id}" data-type="${k}">
                            ...
                            <div class="movie-info">
                              ...
                              <div class="movie-actions">
                               <button class="play-button" onclick="window.location.href='/p/info.html?id=${h.id}&type=${k}'">
                    ...
                    Z = function () {
                        l ||
                            R >= s ||
                            (window.scrollY + window.innerHeight >= document.body.offsetHeight - 400 && (R++, T()));
                    };
                var G = S,
                    ee = T,
                    Q = Z;
        ...
        function K() {
            let P = 0,
                ...
                M = setInterval(() => {
                    window.apiKey
                        ? (clearInterval(M), Y(window.apiKey))
                        : P++ >= J &&
                          (clearInterval(M),
    ...
    document.addEventListener("DOMContentLoaded", () => {
        document.addEventListener("click", function (s) {
            if (
                s.target.classList.contains("scroll-btn") ||
                s.target.classList.contains("cast-scroll-btn") ||
                s.target.classList.contains("tmdb-scroll")
            ) {
                let a = s.target
                    .closest(".cast-scroll-wrapper, .tmdb-wrapper")
                    ?.querySelector(".cast-scroll, .tmdb-row");
                if (a) {
                    let v = s.target.classList.contains("left") ? -1 : 1,
                        E = a.clientWidth;
                    a.scrollBy({ left: v * E, behavior: "smooth" });
                }
            }
        });
        let Y;
      ...
      <li class="bookmark-item">
        ...
        function z(s) {
            ...
            new ResizeObserver(() => {
                ...
                l.addEventListener("scroll", H, { passive: !0 }), H();
            }).observe(l);
        }
        function q(s) {
            function l(L, I) {
            ...
            function a(L, I) {
                return L.media_type && (L.media_type === "movie" || L.media_type === "tv")
                    ? L.media_type
                    : I.startsWith("tv/") || I.startsWith("discover/tv") || I.startsWith("trending/tv")
                      ? "tv"
                      : "movie";
            }
            async function v(L, I) {
                ...
                try {
                    let u = function (e) {
                            (n = e),
                                h.forEach((r, w) => (r.style.display = w === e ? "block" : "none")),
                                o.forEach((r, w) => r.classList.toggle("active", w === e)),
                                (Z.textContent = N[e] || ""),
                                g && clearInterval(g),
                                (g = setInterval(() => b(1), 5e3));
                        },
                        b = function (e) {
                    ...
                    i.forEach((e, r) => {
                        ...
                        let U = y[r],
                            ...
                            te = `${window.location.origin}/p/info.html?id=${e.id}&type=${w}`,
                            ne = document.createElement("a");
                        (ne.className = "slide"),
                            (ne.href = te),
                            (ne.style.backgroundImage = `url(https://image.tmdb.org/t/p/original${W})`),
                            (ne.innerHTML = `<div class="slide-caption">${ae ? `<img class="logo-title" src="${ae}" alt="${A} Logo">` : `<h2>${A}</h2>`}</div>`),
                            S.appendChild(ne),
                            N.push(A);
                        let oe = document.createElement("div");
                        (oe.className = "thumbnail-dot"),
                            oe.setAttribute("data-index", r),
                            (oe.style.backgroundImage = `url(https://image.tmdb.org/t/p/w300${W})`),
                            T.appendChild(oe);
                    });
                    ...
                    console.error("Failed to load slider content:", m),
                        S && (S.innerHTML = "<p style='color:red;'>Error loading slider.</p>");
                }
            }
            function E(L, I) {
                document.querySelectorAll(".widget-content").forEach((S) => {
                    ...
                    (k.className = "tmdb-row"),
                        (k.innerHTML = [...Array(7)]
                            .map(
                                () =>
                                    `<div class="tmdb-card shimmer-card"><div class="tmdb-thumb shimmer"><img src="${I}" class="tmdb-logo" alt="Logo" /></div><div class="tmdb-title shimmer"></div></div>`
                            )
                            .join("")),
                        B.appendChild(k),
                                        ...
                                        try {
                                            ...
                                            k.innerHTML = h
                                                ...
                                                .map((g) => {
                                                    ...
                                                    if (!j) return "";
                                                    let C =
                                                            (g.release_date || g.first_air_date || "").split("-")[0] ||
                                                            "N/A",
                                                        t = g.vote_average?.toFixed(1) || "N/A",
                                                        e = `${window.location.origin}/p/info.html?id=${g.id}&type=${u}`;
                                            ...
                                            (n.className = "tmdb-scroll right"),
                                                (n.innerHTML = "&#8250;"),
                                                B.prepend(o),
                                                B.append(n),
                                                z(B);
                                        } catch (f) {
                                            (k.innerHTML = "<p style='color:red;'>Failed to load data.</p>"),
                                                console.error(f);
                                        }
                                    }
                            },
        ...
        function G(s) {
            let a =
                    document.querySelector("#Text1 .widget-content")?.textContent.trim() ||
            ...
            async function L() {
                ...
                try {
                    let [c, d, m] = await Promise.all([
                            fetch(`${a}/${H}/${x}?api_key=${s}&language=en-US`),
                            fetch(`${a}/${H}/${x}/credits?api_key=${s}&language=en-US`),
                            fetch(`${a}/${H}/${x}/${H === "movie" ? "release_dates" : "content_ratings"}?api_key=${s}`),
                        ]),
                        p = await c.json(),
                        f = await d.json(),
                        i = await m.json(),
                        $ = I(H, i);
                    S(p, f, $), H === "tv" && T(p.seasons), await k();
                    ...
                    if (y && f.cast?.length > 0) {
                        let h = f.cast
                                ...
                                .map(
                                    (n) =>
                                        `<div class="cast-card"><img src="${n.profile_path ? v + n.profile_path : "https://i.imgur.com/obaaZjk.png"}" alt="${n.name}"><div>${n.name}</div><small>${n.character}</small></div>`
                                )
                                .join(""),
                            o = document.createElement("div");
                        (o.className = "cast-scroll-wrapper"),
                            (o.innerHTML = `<button class="scroll-btn left" aria-label="Scroll Left">\u2039</button><div class="cast-scroll">${h}</div><button class="scroll-btn right" aria-label="Scroll Right">\u203A</button>`),
                            y.replaceWith(o),
                            z(o);
                    }
                        ...
                        N &&
                            (o.results && o.results.length > 0
                                ? (N.innerHTML = o.results
                                      ...
                                      .map((n) => {
                                          let g = n.author || "Anonymous",
                                              ...
                                              b = u
                                                  ? u.startsWith("/https")
                                                      ? u.slice(1)
                                                      : `https://image.tmdb.org/t/p/w45${u}`
                                      ...
                                      .join(""))
                                : (N.innerHTML = "<p>No user reviews available.</p>"));
                    } catch {
            ...
            function I(c, d) {
                return c === "movie"
                    ? d.results?.find((p) => p.iso_3166_1 === "US")?.release_dates?.[0]?.certification || "NR"
                    : d.results?.find((p) => p.iso_3166_1 === "US")?.rating || "NR";
            }
            function S(c, d, m) {
            ...
            async function T(c) {
                ...
                (d.innerHTML = ""),
                    (c || [])
                        .filter((f) => f.season_number !== 0)
                        .forEach((f) => {
                            let i = document.createElement("button");
                            (i.className = "ep-btn season-btn"),
                                (i.dataset.seasonNumber = f.season_number),
                                (i.dataset.airDate = f.air_date || ""),
                                (i.dataset.posterPath = f.poster_path || ""),
                                (i.textContent = `S${f.season_number}`),
                                (i.onclick = () => {
                                    document
                                        .querySelectorAll(".season-btn")
                                        .forEach(($) => $.classList.remove("active")),
                                        i.classList.add("active"),
                                        Z(i.dataset.seasonNumber, i.dataset.airDate, i.dataset.posterPath);
                                }),
                                d.appendChild(i);
                        });
            ...
            async function Z(c, d, m) {
                F = c;
                var _gwBtn = document.getElementById("gw-watch-btn");
                if (_gwBtn) _gwBtn.dataset.season = c;
                ...
                let h = await (await fetch(`${a}/tv/${x}/season/${c}?api_key=${s}&language=en-US`)).json(),
                    ...
                    g = (u, b = !1) => {
                        ...
                        if (
                            ...
                            (C.className = `btn episode-btn ${b ? "hide-btn" : "show-more"}`),
                                (C.textContent = b ? "Hide" : "More"),
                                (C.onclick = () => g(h.episodes, !b)),
                                p.appendChild(C);
                        }
                    };
            ...
            async function B(c) {
                ...
                if (f.belongs_to_collection)
                    (
                        (
                            await (
                                await fetch(
                                    `https://api.themoviedb.org/3/collection/${f.belongs_to_collection.id}?api_key=${s}`
                                )
                            ).json()
                        ).parts || []
                    )
                        .sort((h, o) => new Date(h.release_date) - new Date(o.release_date))
                        .forEach((h, o) => {
                            let n = document.createElement("a");
                            (n.className = "ep-btn season-btn" + (h.id == c ? " active" : "")),
                                (n.textContent = `Part ${o + 1}`),
                                (n.href = `${window.location.origin}/p/info.html?id=${h.id}&type=movie`),
                                d.appendChild(n);
                        });
                else {
                ...
                (i.className = "btn episode-btn"),
                    (i.textContent = "01"),
                    (i.href = `/p/player.html?id=${c}&type=movie`),
                    m.appendChild(i);
            }
            async function k() {
            ...
            (window.goToPlayer = function () {
                ...
                if (d && m) {
                    let p =
                        m === "tv"
                            ? `/p/player.html?id=${d}&type=tv&season=${F}&ep=1`
                            : `/p/player.html?id=${d}&type=movie`;
                ...
                (window.bookmarkItem = function () {
                    ...
                    let $ = `/p/info.html?id=${d}&type=${m}`;
                    i.push({ id: d, type: m, title: p, poster: f, url: $ }),
                        localStorage.setItem("abefilm_bookmarks", JSON.stringify(i)),
                        P("Added to Watchlist", "success"),
                        J();
                }),
                document.addEventListener("click", async function (c) {
                    if (c.target.closest(".poster-play-btn")) {
                        ...
                        try {
                            let y;
                            p === "tv" && f
                                ? (y = `${a}/tv/${m}/season/${f}/videos?api_key=${s}&language=en-US`)
                                : (y = `${a}/${p}/${m}/videos?api_key=${s}&language=en-US`);
                            let h = await (await fetch(y)).json(),
                                o =
                                    h.results.find((n) => n.type === "Trailer" && n.site === "YouTube") ||
                                    h.results.find((n) => n.type === "Teaser" && n.site === "YouTube");
                            o
                                ? (i.innerHTML = `<iframe src="https://www.youtube.com/embed/${o.key}?autoplay=1" allowfullscreen allow="autoplay; encrypted-media" frameborder="0"></iframe>`)
                                : (i.innerHTML = "<p>No trailer found for this season.</p>"),
                    ...
                    if (c.target.closest(".share")) {
                        ...
                        navigator.share
                            ? navigator.share({ title: m, text: "Watch this online:", url: d })
                            : navigator.clipboard.writeText(d).then(
                                  () => P("Link copied!", "success"),
                                  () => prompt("Copy link:", d)
                              );
                    }
        ...
        function ee(s) {
            ...
            function x(o, n, g) {
                let u = { server: o, season: n, episode: g };
            ...
            function O(o, n, g) {
                fetch(`https://api.themoviedb.org/3/${n}/${o}?api_key=${s}&language=en-US`)
                    .then((u) => u.json())
                    .then((u) => {
                        let b = u.title || u.name || "Untitled",
                            j = n === "tv" ? ` S${g}` : "";
                        document.title = `${b}${j} - Click Me | AbeFilm`;
                    })
            ...
            (S.id = "season-buttons"),
                S.classList.add("season-button-container"),
                I && I.insertAdjacentElement("afterend", S);
            let T = H(),
                Z = T?.season,
...
`
            ...
            function f(o = 1) {
                ...
                F.innerHTML = g
                    ? `<iframe src="${g}" frameborder="0" allowfullscreen loading="lazy" style="width:100%;height:100%;max-height:100vh;"></iframe>`
                ...
                I &&
                    Object.keys(m).forEach((o) => {
                        ...
                        (n.textContent = o.toUpperCase()),
                            (n.className = "server-btn"),
                            d || (d = o),
                            o === d && n.classList.add("active"),
                            n.addEventListener("click", () => {
                                document.querySelectorAll(".server-btn").forEach((g) => g.classList.remove("active")),
            ...
            function $(o) {
                ...
                for (let u = 1; u <= o; u++) {
                    ...
                    (b.textContent = `${u}`),
                        (b.className = "player-episode-btn"),
                        b.addEventListener("click", () => {
                            document
                                .querySelectorAll(".player-episode-btn")
            ...
            function N(o) {
                ...
                let U = t ? t.toFixed(1) : "N/A",
                    ...
                    te =
                        o.certification ||
                        o.content_ratings?.results?.find((V) => V.iso_3166_1 === "US")?.rating ||
                        "NR";
                ...
                ne &&
                    ...
                    oe.addEventListener("click", () => {
                        ne.classList.toggle("expanded"),
                            (oe.textContent = ne.classList.contains("expanded") ? "Less" : "More");
                    }),
                    fetch(`https://api.themoviedb.org/3/${w ? "tv" : "movie"}/${o.id}/credits?api_key=${s}`)
                        .then((V) => V.json())
                        .then((V) => {
                            ...
                            if (X?.length > 0) {
                                ...
                                (se.className = "footer-cast-scroll-wrapper"),
                                    (se.innerHTML = `<h4>Cast</h4><div class="cast-scroll-wrapper"><button class="cast-scroll-btn left" aria-label="Scroll Left">\u2039</button><div class="cast-scroll">${X.map((re) => `<div class="cast-card"><img src="${re.profile_path ? `https://image.tmdb.org/t/p/w185${re.profile_path}` : "https://i.imgur.com/obaaZjk.png"}" alt="${re.name}" onerror="this.onerror=null;this.src='https://i.imgur.com/obaaZjk.png';"><div class="cast-name">${re.name}</div><div class="cast-role">${re.character}</div></div>`).join("")}</div><button class="cast-scroll-btn right" aria-label="Scroll Right">\u203A</button></div>`),
                                    n.querySelector(".footer-left").appendChild(se),
                                    z(se.querySelector(".cast-scroll-wrapper"));
                            }
                        })
            ...
            function h(o, n) {
                ...
                fetch(u)
                    ...
                    .catch((b) => {
                        console.error("Recommendation Fetch Error:", b),
                            (g.innerHTML = "<p style='color:#ccc'>Failed to load recommendations.</p>");
                    });
            }
            O(a, v, B),
                p(),
                Object.keys(m).length > 0
                    ? (i(), f(c))
                    : F && (F.innerHTML = "<p style='color:white'>No server templates found.</p>"),
                v === "tv"
                    ? (fetch(`https://api.themoviedb.org/3/tv/${a}?api_key=${s}`)
                          .then((o) => o.json())
                          .then((o) => {
                              N(o),
                                  h(o.id, v),
                                  o.seasons &&
                                      o.seasons.length > 0 &&
                                      S &&
                                      (S.innerHTML = o.seasons
                                          .filter((n) => n.season_number !== 0)
                                          .map(
                                              (n) =>
                                                  `<a href="?id=${a}&type=tv&season=${n.season_number}" class="season-btn${n.season_number === B ? " active" : ""}">${n.name}</a>`
                                          )
                                          .join(""));
                          }),
                      fetch(`https://api.themoviedb.org/3/tv/${a}/season/${B}?api_key=${s}`)
                          .then((o) => o.json())
                          .then((o) => {
                              $(o.episodes?.length || 0);
                          }))
                    : v === "movie" &&
                      (fetch(`https://api.themoviedb.org/3/movie/${a}?api_key=${s}`)
                          .then((o) => o.json())
                          .then((o) => {
                              N(o),
                                  h(o.id, v),
                                  o.belongs_to_collection?.id &&
                                      fetch(
                                          `https://api.themoviedb.org/3/collection/${o.belongs_to_collection.id}?api_key=${s}`
                                      )
                                          .then((n) => n.json())
                                          .then((n) => {
                                              n.parts &&
                                                  ...
                                                  (S.innerHTML = n.parts
                                                      .sort(
                                                          (g, u) => new Date(g.release_date) - new Date(u.release_date)
                                                      )
                                                      .map(
                                                          (g, u) =>
                                                              `<a href="?id=${g.id}&type=movie&part=${u + 1}" class="season-btn${g.id == a ? " active" : ""}">${"Part " + (u + 1)}</a>`
                                                      )
                                                      .join(""));
                                          });
                          }),
        ...
        function R() {
            let s = 0,
                ...
                a = setInterval(() => {
                    window.apiKey
                        ? (clearInterval(a), Q(window.apiKey))
                        : s++ >= l &&
                          (clearInterval(a),
                          ...
                          [
                              ...
                              E &&
                                  (E.innerHTML =
                                      "<p style='color:red; text-align:center; padding: 2rem;'>Error: API configuration is missing.</p>");
                          }));
                }, 100);
        }
        R();
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        z && v && H && (v.innerHTML = H.innerHTML),
            q && E && O && (E.innerHTML = O.innerHTML),
            G && x && F && (x.innerHTML = F.innerHTML);
        let L = document.getElementById("details-content"),
            I = document.getElementById("player-content"),
            S = document.getElementById("library");
        L && (L.style.display = M ? "block" : "none"),
            I && (I.style.display = D ? "block" : "none"),
            S && (S.style.display = _ ? "block" : "none"),
            v && (v.style.display = z ? "block" : "none"),
            E && (E.style.display = q ? "block" : "none"),
            x && (x.style.display = G ? "block" : "none"),
            D && (document.querySelector(".top-header")?.remove(), document.querySelector(".side-menu")?.remove()),
            _ && K && document.querySelector(".top-header")?.remove();
        let T = {
            home: "tmdb-section-1",
        ...
        if (
            (document
                .querySelectorAll('.side-menu a[href^="#"], .bottom-navbar a[href^="#"], .anchor-links a[href^="#"]')
                .forEach((i) => {
        ...
        function B(i, $ = "info") {
            ...
            y ||
                ...
                (y.id = "custom-toast-notification"),
                document.body.appendChild(y)),
                clearTimeout(Z),
                (y.textContent = i),
                (y.className = "toast-show " + $),
                (Z = setTimeout(() => {
            ...
            document.querySelector(".share-btn")?.addEventListener("click", () => {
                navigator.share
                    ? navigator
                          ...
                          .catch((i) => console.warn("Share failed:", i))
                    : navigator.clipboard
                          .writeText(window.location.href)
                          .then(() => B("Link copied to clipboard!", "success"))
                          .catch(() => B("Could not copy link.", "error"));
            }),
        ...
        p &&
            ...
            p.addEventListener("click", (i) => {
                i.preventDefault(), f.scrollIntoView({ behavior: "smooth", block: "start" });
            });
    });
    document.addEventListener("DOMContentLoaded", () => {
        if (!document.getElementById("comment-section")) return;
        let { createClient: K } = supabase,
            M = K(
                "https://hwucnfgzeghfmagromyb.supabase.co",
                "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh3dWNuZmd6ZWdoZm1hZ3JvbXliIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ2MzUwMjMsImV4cCI6MjA3MDIxMTAyM30.Sl8JjfFj9iEWZxwun9XzaZeQcEN1BtfcU2FmlGdemI8"
            ),
            D = new URLSearchParams(window.location.search),
            _ = D.get("id"),
            z = D.get("type"),
            q = _ && z ? `${z}-${_}` : window.location.pathname,
            G = document.getElementById("authModal"),
            ee = document.getElementById("closeAuthModalBtn"),
            Q = document.getElementById("showAuthModalBtn"),
            R = document.getElementById("loginPrompt"),
            s = document.getElementById("commentFormContainer"),
            l = document.getElementById("message"),
            a = document.getElementById("submit"),
            v = document.getElementById("logoutBtn"),
            E = document.getElementById("avatarModal"),
            x = document.getElementById("closeAvatarModalBtn"),
            H = document.getElementById("currentUserAvatar"),
            O = document.getElementById("currentUsernameDisplay"),
            F = document.getElementById("saveAvatarBtn"),
            L = null,
            I = "https://cdn-icons-png.flaticon.com/128/1046/1046929.png",
            S = !0,
            T = !1,
            Z = {},
            B = {
        ...
        async function k() {
            ...
            let { data: t, error: e } = await M.from("reactions").select("*").eq("topic", q).single();
            if (e && e.code === "PGRST116") {
                let { data: r, error: w } = await M.from("reactions")
                    .insert({ topic: q, upvote: 0, funny: 0, love: 0, surprised: 0, angry: 0, sad: 0 })
                    .select()
                    .single();
                if (w) {
        ...
        async function d(t) {
            ...
            let e = localStorage.getItem(q),
                r;
            e === t
                ? (localStorage.removeItem(q), (r = M.rpc("decrement_reaction", { topic_name: q, reaction_name: t })))
                : e
                  ? (localStorage.setItem(q, t),
                    (r = M.rpc("switch_reaction", { topic_name: q, old_reaction_name: e, new_reaction_name: t })))
                  : (localStorage.setItem(q, t),
                    (r = M.rpc("increment_reaction", { topic_name: q, reaction_name: t })));
            let { error: w } = await r;
            w &&
                (console.error("Failed to update reaction:", w),
                e ? localStorage.setItem(q, e) : localStorage.removeItem(q)),
                await k(),
                (T = !1);
        }
        async function m(t) {
            if (((L = t?.user || null), L)) {
                ...
                (O.textContent = e),
                    (I = L.user_metadata?.profile_url || "https://cdn-icons-png.flaticon.com/128/1046/1046929.png"),
                    (H.src = I);
        ...
        M.auth.onAuthStateChange((t, e) => {
            ...
            document.getElementById("signInBtn").addEventListener("click", async () => {
                let t = document.getElementById("loginEmail").value,
                    ...
                    { error: r } = await M.auth.signInWithPassword({ email: t, password: e });
            ...
            document.getElementById("signUpBtn").addEventListener("click", async () => {
                ...
                let { data: w, error: A } = await M.auth.signUp({
                    email: t,
                    password: e,
                    options: {
                        data: { username: r, profile_url: "https://cdn-icons-png.flaticon.com/128/1046/1046929.png" },
                    },
                });
                A
                    ? alert(A.message)
            ...
            document.getElementById("guestSignInBtn").addEventListener("click", async () => {
                let { data: t, error: e } = await M.auth.signInAnonymously();
                e ? (console.error("GUEST SIGN-IN FAILED:", e), alert(e.message)) : (G.style.display = "none");
            }),
            v.addEventListener("click", async () => {
                let { error: t } = await M.auth.signOut();
                t && alert(t.message);
            });
        function p() {
        ...
        function f() {
            (E.style.display = E.style.display === "block" ? "none" : "block"),
                document
                    .querySelectorAll("#avatarModal .tab-content img")
                    .forEach((e) => e.classList.remove("selected"));
            let t = document.querySelector(`#avatarModal img[src="${I}"]`);
            t && t.classList.add("selected");
        }
        function i(t) {
            document.querySelectorAll("#authModal .tab-content").forEach((e) => e.classList.remove("active")),
                document.getElementById(t).classList.add("active"),
                document.querySelectorAll("#authModal .tab-btn").forEach((e) => e.classList.remove("active")),
                document.querySelector(`#authModal .tab-btn[data-tab="${t}"]`).classList.add("active");
        }
        function $(t) {
            document.querySelectorAll("#avatarModal .tab-content").forEach((r) => r.classList.remove("active")),
                document.getElementById(t).classList.add("active"),
                document.querySelectorAll("#avatarModal .tab-btn").forEach((r) => r.classList.remove("active")),
                document.querySelector(`#avatarModal .tab-btn[data-tab="${t}"]`).classList.add("active");
        }
        Q.addEventListener("click", p),
            ee.addEventListener("click", p),
            x.addEventListener("click", f),
            H.addEventListener("click", f),
            document
                .querySelectorAll("#authModal .tab-btn")
                .forEach((t) => t.addEventListener("click", () => i(t.dataset.tab))),
            document
                .querySelectorAll("#avatarModal .tab-btn")
                .forEach((t) => t.addEventListener("click", () => $(t.dataset.tab)));
        function y() {
        ...
        F.addEventListener("click", async () => {
            ...
            let { data: t, error: e } = await M.auth.updateUser({ data: { profile_url: I } });
            e
                ? alert("Error saving avatar: " + e.message)
                : (t.user &&
                      ((L.user_metadata.profile_url = t.user.user_metadata.profile_url),
                      (H.src = t.user.user_metadata.profile_url)),
                  f(),
        ...
        async function N() {
            ...
            let A = {
                ...
                { error: W } = await M.from("comments").insert([A]);
            W
                ? (console.error("DATABASE ERROR:", W), alert("Could not post comment. See console for details."))
                : ((l.value = ""), j());
        }
        ...
        async function h(t, e) {
            ...
            let A = r.user_metadata?.profile_url || I,
                { error: W } = await M.from("comments").insert([
                    {
                        user_id: r.id,
                        username: w,
                        message: e,
            ...
            W
                ? (console.error("DATABASE ERROR on reply:", W),
        ...
        g.addEventListener("click", () => {
            ...
            u.querySelectorAll("li").forEach((t) => {
                t.addEventListener("click", () => {
                    u.querySelectorAll("li").forEach((e) => e.classList.remove("active")),
                        t.classList.add("active"),
                        (g.innerText = t.innerText),
                        (u.style.display = "none"),
                        (S = t.dataset.value === "recent"),
                        j();
                });
            }),
        ...
        async function b(t, e) {
            ...
            if (!r) return p();
            let { data: w, error: A } = await M.from("comments").select(e).eq("id", t).single();
            if (A) {
                console.error(`Error fetching ${e} count:`, A);
                return;
            }
            let W = (w[e] || 0) + 1,
                { error: U } = await M.from("comments")
                    .update({ [e]: W })
                    .eq("id", t);
            U
                ? (console.error(`Error updating ${e} count:`, U),
                  alert(`Could not update ${e}. See console for details.`))
                : j();
        }
        async function j() {
            let { data: t, error: e } = await M.from("comments")
                .select("*")
                .eq("topic", q)
                .order("created_at", { ascending: !S });
            if (e) {
                if (e.code === "PGRST116") {
            ...
            let w = t.filter((U) => !U.parent_id),
              ...
              <div class="replies"></div>`);
                    ...
                    return (
                        te.querySelector(".like-btn").addEventListener("click", () => b(U.id, "likes")),
                        te.querySelector(".dislike-btn").addEventListener("click", () => b(U.id, "dislikes")),
                        te.querySelector(".reply-btn").addEventListener("click", () => {
                            if (((V.innerHTML = ""), !L)) return p();
                            let X = document.createElement("div");
                            (X.innerHTML = `<textarea placeholder="What's on your mind?" class="reply-message"></textarea><button class="comment-btn reply-submit">Submit Reply</button>`),
                                V.appendChild(X),
                                X.querySelector(".reply-submit").addEventListener("click", async () => {
        ...
        (async () => {
            ...
            await m(t),
                document.querySelectorAll(".reaction").forEach((e) => {
